...
llvm::Value *Method::generateCode(Program *program, Class* cls, const std::string &fileName){
    ...
    for(auto formal: getFormals())
    {
        llvm::Value *arg = llvm->builder->CreateAlloca(llvm->getType(formalsMap[formal->getName()]->getType()));
...
Call::Call(Expr *objExpr, const string methodName, Args *listExpr, const int line, const int column): objExpr(objExpr), methodName(methodName), listExpr(listExpr){
    ...
    this->column = column;
    if(!Call::objExpr)
        Call::objExpr = new Self(line, column, new ObjectIdentifier("self", line, column));
}
...
llvm::Value *Call::generateCode(Program *program, Class* cls, const std::string &fileName){
    ...
    auto typeLLVM = llvm->mdl->getTypeByName(objExpr->type + "VTable");
    auto method = llvm->builder->CreateStructGEP(typeLLVM, table, iMethod);
    method = llvm->builder->CreateLoad(method);
    auto methodType = (llvm::FunctionType*) ((llvm::PointerType*) typeLLVM->getStructElementType(iMethod))->getElementType();
    auto argType = methodType->getParamType(0);
    arg = llvm->builder->CreatePointerCast(arg, argType);
...
Self::Self(const int line, const int column, ObjectIdentifier *identifier): identifier(identifier){
    this->line = line;
    this->column = column;
}
...
llvm::Value *Self::generateCode(Program *program, Class* cls,const std::string &fileName){
    return identifier->generateCode(program, cls, fileName);
}
